...
δz = 20. # interface smoothing scale
...
def bottom_frontal_zone_basic(z, M, Hbfz=Hbfz, θ=θ, f=f, δz=δz, derivatives=False):
    B = np.zeros_like(z)
    U = np.zeros_like(z)
    V = np.zeros_like(z)
    ...
    if not derivatives:
        ...
        Bz = -M**2/np.sin(θ) * sigmoid(z, z0=Hbfz, δz=δz)
        Vz = M**2 / (f * np.cos(θ)) * sigmoid(z, z0=Hbfz, δz=δz)
        
        return B, U, V, Bz, Uz, Vz
